...
public abstract class AbstractTreeGenericXYCommonXDataProvider<A extends TmfStateSystemAnalysisModule, M extends ITmfTreeDataModel>
    ...
    public TmfModelResponse<ITmfXyModel> fetchXY(Map<String, Object> fetchParameters, @Nullable IProgressMonitor monitor) {
        ...
        try (FlowScopeLog scope = new FlowScopeLogBuilder(LOGGER, Level.FINE, "AbstractTreeGenericXYCommonXDataProvider#fetchXY") //$NON-NLS-1$
            ...
            // Auto-enhance with mipmap max-per-bucket where available (time-based
            // sampling only). Opt-in per provider (disabled by default).
            ISampling sampling = xAxisAndYSeriesModels.getFirst();
            Collection<IYModel> yModels = xAxisAndYSeriesModels.getSecond();
            if (isMipmapEnhanced() && sampling instanceof ISampling.Timestamps timestamps) {
                yModels = MipmapXYQueryHelper.enhanceWithMipmap(ss, getIdToQuark(), yModels, timestamps.timestamps());
            }
            return TmfXyResponseFactory.create(
                    getTitle(),
                    sampling,
                    ImmutableList.copyOf(yModels),
                    getDisplayType(),
                    getXAxisDescription(),
                    complete);
