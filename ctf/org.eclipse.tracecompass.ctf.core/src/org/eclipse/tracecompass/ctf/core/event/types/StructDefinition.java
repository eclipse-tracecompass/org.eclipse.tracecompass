...
public final class StructDefinition extends ScopedDefinition implements ICompositeDefinition {
     ...
     */
    @Override
    public Definition lookupDefinition(String lookupPath, ScopedDefinition definitionToExclude) {
        if (this.equals(definitionToExclude) || lookupPath == null) {
            return null;
        }
        /*
        ...
        for (IDefinition child : fDefinitions) {
            if (child instanceof ScopedDefinition scopedChild) {
                if (!child.equals(definitionToExclude)) {
                    if (lookupPath.equals(child.getDeclaration().getRole())) {
                        return scopedChild;
                    }
                    IDefinition def = scopedChild.lookupDefinition(lookupPath, definitionToExclude);
                    if (def instanceof Definition childDef) {
                        return childDef;
                    }
                }
            }
